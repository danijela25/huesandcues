...
func _store_guess_markers(guesses):
	...
	for g in guesses:
		var player_id := str(g.get("playerId", ""))
		var idx: int = int(player_index_map.get(player_id, 1))

		guess_markers.append({
			"x": int(g.get("x", 0)),
			"y": int(g.get("y", 0)),
			"symbol": str(idx) + ("²" if str(g.get("type", "first")) == "second" else "¹"),
			"type": str(g.get("type", "first"))
		})
